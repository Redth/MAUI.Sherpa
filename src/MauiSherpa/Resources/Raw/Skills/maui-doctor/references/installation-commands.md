...
```bash
# MAUI build shortcuts
alias maui-build='dotnet build -f net9.0-android'
alias maui-run='dotnet build -f net9.0-android -t:Run'
alias maui-install='dotnet build -f net9.0-android -t:Install'

# iOS shortcuts (macOS)
alias maui-ios='dotnet build -f net9.0-ios'
alias maui-mac='dotnet build -f net9.0-maccatalyst'

# Emulator shortcuts
alias emu='emulator -avd Pixel_8_API_35 &'
...
**macOS**: While waiting for Xcode to install or update from App Store:
1. Install .NET SDK
2. Install workloads
3. Install Microsoft OpenJDK
4. Download Android SDK
5. ← Xcode finishes, accept license, continue

**Time saved**: ~15-20 minutes (Xcode download happens in background)
...
```powershell
# MAUI shortcuts
function maui-build { dotnet build -f net9.0-android }
function maui-run { dotnet build -f net9.0-android -t:Run }
function maui-win { dotnet build -f net9.0-windows10.0.19041.0 }
